...
produce_catch_map_decade = function(data, decade, fill_by, show_scatterpie_legend = FALSE) {
  return(produce_map_decade(data, decade, CATCH, fill_by, show_scatterpie_legend = show_scatterpie_legend))
}

produce_catch_map_year = function(data, year, fill_by, show_scatterpie_legend = FALSE) {
  return(produce_map_year(data, year, CATCH, fill_by, show_scatterpie_legend = show_scatterpie_legend))
}
...
mix_colors = function(color1, color2, alpha = .5) {
  #### THIS IS F*CKING INSANE!!! AND THE IDIOT RESPONSIBLE FOR FORCING 
  #### ME TO DO THE BELOW SHALL BE CURSED IN PERPETUITY!!!
  
  #### I JUST NEEDED TO MIX TWO COLORS PROVIDED AS #rrggbb YOU M*THERF*CKER
  
  C1 = col2rgb(color1)
  C2 = col2rgb(color2)
  
  C1 = RGB(C1[1] / 255, C1[2] / 255, C1[2] / 255)
  C2 = RGB(C2[1] / 255, C2[2] / 255, C2[2] / 255)
  
  C3 = mixcolor(alpha, C1, C2)
  
  return(rgb(C3@coords[1], C3@coords[2], C3@coords[3]))
}
