...
dtfunction(ESPECES_PRIMAIRES) %>% formatStyle(columns = c("ESPECE_SCIENTIFIQUE"), fontStyle = "italic")
```
...
dtfunction(ESPECES_AUTRES) %>% formatStyle(columns = c("ESPECE_SCIENTIFIQUE"), fontStyle = "italic")
```
...
dtfunction(ETP_SPECIES) %>% formatStyle(columns = c("ESPECE_SCIENTIFIQUE"), fontStyle = "italic")
```
...
Les pêcheries de la CTOI proviennent de la combinaison de plusieurs facteurs : (i) le [but de la pêche](../domaine/pêche/#fisheryPurposes), (ii) la [zone d'opération](../domaine/pêche/#fisheryAreasOfOperation), (iii) la [gamme de taille des navires](../latest/domaine/pêche/#fisheryLOAClasses), (iv) le [groupe d'engins](../domaine/pêche/#gearGroups), (v) l'[engin](../domaine/pêcheries/#gears), (vi) la [configuration de l'engin](../domaine/pêcheries/#gearConfigurations), (vii) le [mode de pêche](../domaine/pêcheries/#fishingModes) dans le cas des sennes coulissantes, des hameçons et des lignes, et des cannes et lignes, et (viii) l'[espèce cible](../domaine/pêcheries/#targetSpecies). L'[assistant d'identification des pêcheries de la CTOI](https://data.iotc.org/fisheries/wizard/fr/) a été conçu pour guider l'adoption de la nouvelle classification des pêcheries en fournissant un outil interactif qui génère le code et la définition de la pêcherie sur la base des facteurs susmentionnés.
